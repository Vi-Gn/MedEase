...
class MainMenu(TMenu):
  def __init__(self, appRoot, *args, **vargs) -> None:
    ...
    self.file_menu.add_separator()
    self.file_menu.add_command(label="Reload Database (F5)", command= lambda : self.app.framedTableData.LoadDataTable())
    self.file_menu.add_command(label="Reload Directory (F5)", command= lambda : self.app.framedTableFile.ReLoadDirectoryTable())
    self.file_menu.add_separator()
    self.file_menu.add_command(label="Exit (Alt+F4)", command=self.app.Quit)
    self.add_cascade(label="File", menu=self.file_menu)

    self.edit_menu = TMenu(self, tearoff=0)
    self.edit_menu.add_command(label="Add Item (Ins)", command = lambda : self.app.framedTableData.AddItem())
    self.edit_menu.add_command(label="Remove Item (Del)", command = lambda : self.app.framedTableData.RemoveSelectedItems())
    ...
    self.help_menu.add_command(label="Documentation (F1)", command= lambda : self.app.OpenDocumentation())
